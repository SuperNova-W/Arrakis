...
NewsFeatureSnapshot PostgresPool::news_snapshot(std::string_view symbol, std::string_view trading_date, std::size_t article_limit) const {
    ...
    require_result(daily.get(), PGRES_TUPLES_OK);
    if (PQntuples(daily.get()) > 0) {
    ...
    Result articles(PQexecParams(lease.connection, "SELECT a.article_id,a.canonical_url,a.source_id,a.headline,a.body,EXTRACT(EPOCH FROM a.published_at)*1000,EXTRACT(EPOCH FROM a.retrieved_at)*1000,a.novelty_score,COALESCE(n.positive_probability,0),COALESCE(n.neutral_probability,0),COALESCE(n.negative_probability,0),COALESCE(n.sentiment_score,0) FROM news_articles a LEFT JOIN news_nlp_features n ON n.article_id=a.article_id WHERE EXISTS (SELECT 1 FROM news_article_entities e WHERE e.article_id=a.article_id AND e.entity_type='etf' AND e.entity_id=$1) AND a.published_at >= ($2::date::timestamp AT TIME ZONE 'America/New_York') AND a.published_at <= (SELECT cutoff_timestamp FROM etf_daily_news_features WHERE symbol=$1 AND trading_date=$2::date) ORDER BY a.published_at DESC,a.article_id LIMIT $3", 3, nullptr, article_values, nullptr, nullptr, 0));
    // Query errors must never look like a successfully retrieved empty news day.
    require_result(articles.get(), PGRES_TUPLES_OK);
    {
