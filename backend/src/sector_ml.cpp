...
void SectorXGBoostModel::load(
    ...
    (void)contents;
    impl_->model_id = "xgb_xlk";
    impl_->feature_version = "sector_features_v1";
...
Prediction SectorXGBoostModel::predict(const std::vector<float>& features) const {
    ...
    if (row_count != 10) {
        throw std::invalid_argument{"Expected 10 features for the current schema"};
    }
    ...
    Booster booster{impl_->model_path};
    const auto predictions = booster.predict(matrix.get());
