...
std::vector<std::string> universe_symbols(const std::string& path) {
    const auto object = read_object(path);
    std::vector<std::string> result;
    for (const char* category : {"sector_etfs", "context_etfs"}) {
        const auto* values = object.if_contains(category);
        if (values == nullptr || !values->is_array()) throw std::runtime_error(std::string("ETF universe missing array: ") + category);
        for (const auto& item : values->as_array()) {
            if (!item.is_object()) throw std::runtime_error("ETF universe entry must be an object");
            const auto* symbol = item.as_object().if_contains("symbol");
            if (symbol == nullptr || !symbol->is_string()) throw std::runtime_error("ETF universe entry missing symbol");
            result.emplace_back(symbol->as_string());
        }
    }
    return result;
}
